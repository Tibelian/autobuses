...
public class Operacion {
    ...
    public void guardarViajeBackup(SessionFactory sessionBuilder, Viaje viaje){
        Session session = sessionBuilder.openSession();
        Transaction tx = null;
        String hql = "FROM ViajeroBackup b WHERE b.dni IN (SELECT v.dni FROM Viajero v)";
        Query q = session.createQuery(hql);
        List<ViajeroBackup> viajerosBackupExistentes = q.list();
        try{
            ...
            while(itCompra.hasNext()){
                ...
                while(itOcupacion.hasNext()){
                    ...
                    // le indica el id para que haga update en vez de insert
                    for(ViajeroBackup vBackup : viajerosBackupExistentes){
                        if(vBackup.getDni().equals(viajeroBackup.getDni())){
                            viajeroBackup.setId(vBackup.getId());
                        }
                    }
            ...
            session.clear(); // PARA EVITAR EXCEPTION
    ...
    public void borrarViaje(SessionFactory sessionBuilder, Viaje viaje){
        ...
        try{
            ...
            // elimina viajeros que no tengan ocupación
            String hql = ("FROM Viajero v WHERE v.id NOT IN (SELECT o.viajero FROM Ocupacion o)");
            Query q = session.createQuery(hql);
            List viajeros = q.list();
